...
namespace FluxCommerce.Api.Application.Handlers.Chat;
...
public class ProcessChatMessageCommandHandler : IRequestHandler<ProcessChatMessageCommand, string>
{
    private readonly Kernel _kernel;
    private readonly IChatCompletionService _chatService;
    private readonly IMediator _mediator;

    public ProcessChatMessageCommandHandler(Kernel kernel, IMediator mediator)
    {
        _kernel = kernel;
    ...
    public async Task<string> Handle(ProcessChatMessageCommand request, CancellationToken cancellationToken)
    {
        ...
        var processedResponse = await ProcessAIResponse(aiResponse, request.UserId, request.StoreId, cancellationToken);
        ...
        try
        {
            ...
            switch (jsonResponse.Action?.ToLower())
            {
                case "search":
                    ...
                    var products = await GetProductSearchResults(jsonResponse.Query ?? "", storeId, cancellationToken);
    ...
    // Replace the entire HandleProductSearch method with this:

    private async Task<List<ProductSearchResult>> GetProductSearchResults(string query, string storeId, CancellationToken cancellationToken)
    {
        try
        {
        ...
        {
            Console.WriteLine($"💥 DEBUG: Error in GetProductSearchResults: {ex.Message}");
            Console.WriteLine($"💥 DEBUG: Stack trace: {ex.StackTrace}");
