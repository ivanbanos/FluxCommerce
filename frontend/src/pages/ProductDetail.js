  ...
  return (
    <div style={{ maxWidth: 800, margin: '0 auto', padding: 24 }}>
      <div style={{ display: 'flex', gap: 32 }}>
        <div>
          <img src={product.images?.[product.coverIndex] || '/placeholder.png'} alt={product.name} style={{ width: 320, height: 320, objectFit: 'cover', borderRadius: 8 }} />
          <div style={{ display: 'flex', gap: 8, marginTop: 8 }}>
            {product.images?.map((img, idx) => (
              <img key={idx} src={img} alt='' style={{ width: 60, height: 60, objectFit: 'cover', borderRadius: 4, border: idx === product.coverIndex ? '2px solid #1976d2' : '1px solid #ccc' }} />
            ))}
          </div>
        </div>
        <div style={{ flex: 1 }}>
